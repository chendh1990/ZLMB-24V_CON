...
static void SetTxPacketData(uint8 const XDATA* const d, uint8 XDATA length);
static void SendTxPacketData(uint8 XDATA cmd);


void UartPacketInit(void)
{
	isrCfg.rxCnt = 0;
	isrCfg.rxStatus = RX_IDDLE;
	isrCfg.rxPack.head_0 = P_HEAD_0;
	isrCfg.rxPack.head_1 = P_HEAD_1;
	isrCfg.rxPack.ver = P_VER;
	
	isrCfg.txCnt = 0;
	isrCfg.txStatus = RX_IDDLE;
	isrCfg.txPack.head_0 = P_HEAD_0;
	isrCfg.txPack.head_1 = P_HEAD_1;
	isrCfg.txPack.ver = P_VER;
}
void UartPacketParse(void)
{
	...
	switch(isrCfg.rxStatus)
	{
		...
		case RX_LEN_0:
		{
			...
			if(isrCfg.rxLength <= (UART_TX_MAX))
			{
		...
		//	checkSum = getCheckSum((uint8  *)(&(isrCfg.rxPack)), isrCfg.rxCnt+3) + SUM_HEAD_VER;
			...
			if(checkSum == dat)
			{
				...
				UartPackethandle();
			//	UartSendHexBuffer(UART_WIFI, "+raw:", (uint8  *)(&(isrCfg.rxPack.cmd)), isrCfg.rxCnt+3, ' ', true);
...
static void SetTxPacketData(uint8 const XDATA* const d, uint8 XDATA length)
{
	my_memcpy(&isrCfg.txPack.dat, d, length);
	isrCfg.txLength = length;
}
static void SendTxPacketData(uint8 XDATA cmd)
{
	uint8 XDATA txLength = isrCfg.txLength+6;
	isrCfg.txPack.cmd = cmd;
	isrCfg.txPack.length[0] = HI_UINT16(isrCfg.txLength);
	isrCfg.txPack.length[1] = LO_UINT16(isrCfg.txLength);
	isrCfg.txPack.dat[isrCfg.txLength] = getCheckSum((uint8 const *)&isrCfg.txPack.head_0, txLength++);
//	UartSendBuffer(UART_WIFI, (const uint8 *const)&isrCfg.txPack.head_0, txLength);
	UartSendHexBuffer(UART_WIFI, "res:", (const uint8 *const)&isrCfg.txPack.head_0, txLength, ' ', true);
}
