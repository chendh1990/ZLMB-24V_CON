...
static uint8 dpDownloadHandle(uint8 dpid, const uint8 value[], uint8 length);
...
使用说明 : 该函数用户不能修改
*****************************************************************************/
uint8 dpDownloadHandle(uint8 XDATA dpid, const uint8 *val, uint8 XDATA length)
{
  /*********************************
  当前函数处理可下发/可上报数据调用                    
  具体函数内需要实现下发数据处理
...
static uint8 DPHand(uint8 XDATA* pDPVal)
{
	uint8 XDATA dpID;
	uint8 XDATA dpIndex;
	uint8 XDATA dpType;
	...
	{
		return DPDownloadHandle(dpID, pVal, dpLength);
	}
	return ret;
}
static void SendTxPacketData(uint8 XDATA cmd)
