...
void dpBoolUpdate(uint8 XDATA *const pbuffer, uint8 dpid, uint8 value)
{
	if(!pbuffer)
	{
		return ;
...
static void AllDataUpdate(void)
{
	...
	dpBoolUpdate(buffer, DPID_SWITCH_1, 1);
	dpBoolUpdate(buffer+5, DPID_SWITCH_2, 1);
	SetTxPacketData(buffer, 10);
}
